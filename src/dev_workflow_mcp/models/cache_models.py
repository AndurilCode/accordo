...
class CacheMetadata(BaseModel):
    ...
    workflow_file: str | None = Field(default=None, description="Path to workflow YAML file")
...
class CachedWorkflowState(BaseModel):
    ...
    embedding_text: str = Field(description="Text used for semantic embedding generation")
...
class CacheSearchQuery(BaseModel):
    ...
    workflow_name: str | None = Field(default=None, description="Filter by workflow name")
    status_filter: list[str] | None = Field(default=None, description="Filter by status values")
    min_similarity: float = Field(default=0.1, description="Minimum similarity score")
    max_results: int = Field(default=50, description="Maximum number of results")
    include_inactive: bool = Field(default=True, description="Include inactive/completed sessions")
...
class CacheStats(BaseModel):
    ...
    oldest_entry: datetime | None = Field(default=None, description="Timestamp of oldest entry")
    newest_entry: datetime | None = Field(default=None, description="Timestamp of newest entry")

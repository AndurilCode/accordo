...
def customize_configuration(builder: ConfigurationBuilder) -> ConfigurationBuilder:
    ...
    typer.echo("You can modify the following options (press Enter to keep current values):")
    typer.echo()
    
    # Repository path
    current_repo_path = next((opt.value for opt in builder.options if opt.flag == "--repository-path"), ".")
    ...
    # Local state file
    has_local_state = any(opt.flag == "--enable-local-state-file" for opt in builder.options)
    enable_local_state = typer.confirm("Enable local state file storage?", default=has_local_state)
    
    if enable_local_state:
        current_format = next((opt.value for opt in builder.options if opt.flag == "--local-state-file-format"), "JSON")
        format_choice = typer.prompt("State file format (JSON/MD)", default=current_format)
        builder.enable_local_state_file(format_choice.upper())
        
        # Session retention
        current_retention = next((opt.value for opt in builder.options if opt.flag == "--session-retention-hours"), "72")
        retention_hours = typer.prompt("Session retention hours", default=int(current_retention), type=int)
        builder.set_session_retention(retention_hours)
    
    # Cache mode
    has_cache = any(opt.flag == "--enable-cache-mode" for opt in builder.options)
    enable_cache = typer.confirm("Enable cache mode for semantic search?", default=has_cache)
    
    if enable_cache:
        current_model = next((opt.value for opt in builder.options if opt.flag == "--cache-embedding-model"), "all-MiniLM-L6-v2")
        embedding_model = typer.prompt("Embedding model", default=current_model)
        builder.enable_cache_mode(embedding_model)
        
        # Cache path
        current_cache_path = next((opt.value for opt in builder.options if opt.flag == "--cache-db-path"), ".workflow-commander/cache")
        cache_path = typer.prompt("Cache database path", default=current_cache_path)
        builder.set_cache_path(cache_path)
        
        # Max results
        current_max_results = next((opt.value for opt in builder.options if opt.flag == "--cache-max-results"), "50")
        max_results = typer.prompt("Maximum search results", default=int(current_max_results), type=int)
...
def build_custom_configuration() -> ConfigurationBuilder:
    ...
    if typer.confirm("Enable cache mode for semantic search?", default=True):
        ...
        cache_path = typer.prompt("Cache database path", default=".workflow-commander/cache")
        builder.set_cache_path(cache_path)
...
def get_workflow_commander_details() -> tuple[str, MCPServer]:
    ...
    if template is None:
        ...
        if typer.confirm("Would you like to customize this configuration?", default=False):

...
def configure(
    platform: str | None = typer.Option(
        None, "--platform", "-p", 
        help="Target platform (cursor, claude, vscode)"
    ),
    ...
    try:
        ...
        if not non_interactive:
            ...
            typer.echo(f"Command: {server_config.command} {' '.join(server_config.args)}")
...
def list_servers(
    ...
    try:
        ...
        if not servers:
            typer.secho(f"📭 No MCP servers configured for {platform_info.name}", fg=typer.colors.YELLOW)
            typer.echo(f"Configuration file: {final_config_path}")
        else:
            typer.secho(f"📋 Configured MCP servers for {platform_info.name}:", bold=True)
            typer.echo(f"Configuration file: {final_config_path}")
            typer.echo()
...
def remove_server(
    ...
    try:
        ...
        # Confirmation
        if not force:
            typer.secho(f"⚠️ Remove server '{server_name}' from {platform_info.name}?", fg=typer.colors.YELLOW)
            typer.echo(f"Configuration file: {final_config_path}")
        ...
        if success:
            typer.secho(f"✅ Server '{server_name}' removed successfully!", fg=typer.colors.GREEN)
        else:
...
def bootstrap_rules(
    ...
    force: bool = typer.Option(
        False, "--force", "-f",
        help="Overwrite existing content if present"
    )
):
    ...
    try:
        ...
        if success_count == total_count:
            typer.secho("✅ All deployments completed successfully!", bold=True, fg=typer.colors.GREEN)
        else:
            typer.secho("⚠️ Some deployments encountered issues. Check output above.", bold=True, fg=typer.colors.YELLOW)
            raise typer.Exit(1)

...
class BaseConfigHandler(ABC):
    ...
    def validate_server_config_raw(self, server_name: str, server_config_dict: dict[str, Any]) -> list[str]:
    ...
    def save_config(self, config: dict[str, Any], config_path: Path, create_backup: bool = True) -> bool:
    ...
    def get_config_path(self, use_global: bool = True, project_root: Path | None = None) -> Path:
        ...
        if use_global:
            ...
            if project_path is None:
                raise ValueError(f"Platform {self.platform} does not support project-specific configuration")
            return project_path
    ...
    def configure_server(self, server_name: str, server_config: MCPServer, config_path: Path) -> bool:

...
from src.dev_workflow_mcp.models.yaml_workflow import WorkflowDefinition, WorkflowTree, WorkflowNode
...
class TestHandleDynamicWorkflow:
    ...
    def test_handle_dynamic_workflow_next_action_with_valid_choice(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        """Test next action with valid node choice."""
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.parse_criteria_evidence_context') as mock_parse, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.update_dynamic_session_node') as mock_update, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.add_log_to_session') as mock_log, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.get_session') as mock_get_session, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.format_enhanced_node_status') as mock_format:
            ...
            result = _handle_dynamic_workflow(
                mock_session, mock_workflow_def, "next", '{"choose": "middle"}', mock_engine, mock_loader
            )
            
            # Verify calls
            mock_parse.assert_called_once_with('{"choose": "middle"}')
            mock_update.assert_called_once_with("test-session-123", "middle", mock_workflow_def)
    ...
    def test_handle_dynamic_workflow_next_action_invalid_node(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
    ...
    def test_handle_dynamic_workflow_next_action_invalid_transition(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        """Test next action with invalid transition."""
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.parse_criteria_evidence_context') as mock_parse:
            mock_parse.return_value = ("invalid_node", {}, False)
            
            result = _handle_dynamic_workflow(
                mock_session, mock_workflow_def, "next", '{"choose": "invalid_node"}', mock_engine, mock_loader
            )
    ...
    def test_handle_dynamic_workflow_next_action_needs_approval(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        ...
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.parse_criteria_evidence_context') as mock_parse:
            ...
            mock_workflow_def.workflow.tree["start"].next_allowed_nodes = ["middle", "approval", "end"]
            
            result = _handle_dynamic_workflow(
                mock_session, mock_workflow_def, "next", '{"choose": "approval"}', mock_engine, mock_loader
            )
    ...
    def test_handle_dynamic_workflow_next_action_with_approval(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        ...
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.parse_criteria_evidence_context') as mock_parse, \
            ...
            result = _handle_dynamic_workflow(
                mock_session, mock_workflow_def, "next", '{"choose": "approval", "user_approval": true}', mock_engine, mock_loader
            )
            
            # Verify calls
            mock_update.assert_called_once_with("test-session-123", "approval", mock_workflow_def)
            assert result == "Approval node status"

    def test_handle_dynamic_workflow_next_action_with_criteria_evidence(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        """Test next action with criteria evidence generation."""
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.parse_criteria_evidence_context') as mock_parse, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.generate_node_completion_outputs') as mock_generate, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.update_dynamic_session_node') as mock_update, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.add_log_to_session') as mock_log, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.get_session') as mock_get_session, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.format_enhanced_node_status') as mock_format:
            ...
            # Setup mocks
            criteria_evidence = {"initialized": "system ready", "validated": "requirements complete"}
            mock_parse.return_value = ("middle", criteria_evidence, False)
            mock_get_session.return_value = mock_session
            mock_format.return_value = "Middle node status"
            
            result = _handle_dynamic_workflow(
                mock_session, mock_workflow_def, "next", '{"choose": "middle", "criteria_evidence": {...}}', mock_engine, mock_loader
            )
            
            # Verify criteria evidence generation was called
            mock_generate.assert_called_once_with(
                "start", mock_workflow_def.workflow.tree["start"], mock_session, criteria_evidence
            )
    ...
    def test_handle_dynamic_workflow_next_action_auto_evidence_extraction(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        """Test automatic evidence extraction when no criteria evidence provided."""
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.parse_criteria_evidence_context') as mock_parse, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.extract_automatic_evidence_from_session') as mock_extract, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.update_dynamic_session_node') as mock_update, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.add_log_to_session') as mock_log, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.get_session') as mock_get_session, \
             patch('src.dev_workflow_mcp.prompts.phase_prompts.format_enhanced_node_status') as mock_format:
    ...
    def test_handle_dynamic_workflow_next_action_missing_choice(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
    ...
    def test_handle_dynamic_workflow_next_action_target_node_not_found(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
    ...
    def test_handle_dynamic_workflow_current_node_display(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
        """Test displaying current node status for non-next actions."""
        with patch('src.dev_workflow_mcp.prompts.phase_prompts.format_enhanced_node_status') as mock_format:
            ...
            mock_format.assert_called_once_with(
                mock_workflow_def.workflow.tree["start"], mock_workflow_def, mock_session
            )
            assert result == "Current node status display"

    def test_handle_dynamic_workflow_current_node_not_found(self, mock_session, mock_workflow_def, mock_engine, mock_loader):
...
class TestHandleCacheRestoreOperation:
    ...
    def test_handle_cache_restore_basic_functionality(self):
        ...
        assert any(marker in result for marker in ["✅", "❌", "Cache", "cache", "Error", "error"])
...
class TestHandleCacheListOperation:
    ...
    def test_handle_cache_list_basic_functionality(self):
        ...
        assert any(marker in result for marker in ["📭", "📋", "No cached sessions", "Cached Sessions"])

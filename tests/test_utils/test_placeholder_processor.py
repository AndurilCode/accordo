...
class TestProcessWorkflowContent:
    ...
    def test_workflow_processing(self):
        ...
        assert (
            result["workflow"]["tree"]["analyze"]["goal"]
            == "**ANALYZE:** Implement OAuth"
        )
        assert (
            result["workflow"]["tree"]["analyze"]["acceptance_criteria"]["config_check"]
            == "Read .workflow-commander/project_config.md completely"
        )
    ...
    def test_nested_structure_processing(self):
        ...
        workflow = {
            "levels": {
                "level1": {
                    "level2": {"content": "Task: ${{ inputs.task_description }}"}
                }
            },
            "list_content": [
        ...
        assert (
            result["levels"]["level1"]["level2"]["content"] == "Task: Deep nesting test"
        )
    ...
    def test_empty_inputs_no_processing(self):
        ...
        workflow = {"goal": "Task: ${{ inputs.task_description }}"}
    ...
    def test_original_workflow_unchanged(self):
        ...
        original_workflow = {"goal": "Task: ${{ inputs.task_description }}"}

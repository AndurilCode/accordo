...
def mock_embedding_model():
    ...
    mock_model = Mock()
    mock_model.encode.return_value = [[0.1, 0.2, 0.3] for _ in range(384)]  # Mock 384-dim embeddings
    return mock_model
...
class TestWorkflowCacheManagerInitialization:
    ...
    @patch('chromadb.PersistentClient')
    def test_ensure_initialized_creates_directory(self, mock_client_class, temp_cache_dir):
...
class TestWorkflowCacheManagerEmbeddingModel:
    ...
    @patch('src.dev_workflow_mcp.utils.cache_manager.WorkflowCacheManager._is_test_environment')
    ...
    @patch('src.dev_workflow_mcp.utils.cache_manager.WorkflowCacheManager._is_test_environment')
    @patch('sentence_transformers.SentenceTransformer')
    def test_get_embedding_model_fallback_chain(self, mock_transformer_class, mock_is_test):
        ...
        cache_manager = WorkflowCacheManager("/tmp/test", embedding_model="custom-model")
        result = cache_manager._get_embedding_model()
...
class TestWorkflowCacheManagerFullIntegration:
    ...
    def test_basic_workflow_lifecycle(self, temp_cache_dir, test_workflow_state):
        ...
        with patch('src.dev_workflow_mcp.utils.cache_manager.WorkflowCacheManager._is_test_environment', return_value=True):
            cache_manager = WorkflowCacheManager(str(temp_cache_dir))
            ...
            if store_result and store_result.success:
                ...
                retrieved = cache_manager.retrieve_workflow_state(test_workflow_state.session_id)
                if retrieved:
                ...
                delete_result = cache_manager.delete_workflow_state(test_workflow_state.session_id)
                if delete_result:
...
class TestWorkflowCacheManagerStoreRetrieve:
    ...
    @patch('chromadb.PersistentClient')
    def test_store_workflow_state_not_initialized(self, mock_client_class, test_workflow_state):
    ...
    @patch('chromadb.PersistentClient')  
    def test_store_workflow_state_no_embedding_model(self, mock_client_class, temp_cache_dir, test_workflow_state):
    ...
    @patch('chromadb.PersistentClient')
    def test_store_workflow_state_exception(self, mock_client_class, temp_cache_dir, test_workflow_state):
...
class TestWorkflowCacheManagerSemanticSearch:
    ...
    def test_semantic_search_no_embedding_model(self, temp_cache_dir):
        ...
        with (patch.object(cache_manager, '_ensure_initialized', return_value=True),
...
class TestWorkflowCacheManagerAdditionalMethods:
    ...
    @patch('chromadb.PersistentClient')
    def test_find_similar_workflows_not_initialized(self, mock_client_class, test_workflow_state):

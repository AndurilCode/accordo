...
from src.dev_workflow_mcp.models.yaml_workflow import WorkflowDefinition, WorkflowNode, WorkflowInput, WorkflowTree
from src.dev_workflow_mcp.services.config_service import ServerConfiguration
from src.dev_workflow_mcp.services.session_repository import SessionRepository
from src.dev_workflow_mcp.services.session_sync_service import SessionSyncService
...
class TestSessionRestorationIntegration:
    ...
    def test_complete_session_restoration_workflow(
        self, session_repository, session_sync_service, cache_manager, sample_workflow_def
    ):
        ...
        for client_id, task_description in session_data:
            ...
            print(f"  ✅ Created session {session.session_id[:8]}... for client '{client_id}'")
        ...
        for session in test_sessions:
            success = session_sync_service.sync_session_to_cache(session.session_id, session)
            if success:
                cache_sync_count += 1
                print(f"  ✅ Synced session {session.session_id[:8]}... to cache")
            else:
                print(f"  ❌ Failed to sync session {session.session_id[:8]}... to cache")
        
        assert cache_sync_count == len(test_sessions), f"Expected {len(test_sessions)} sessions synced, got {cache_sync_count}"
        ...
        assert cache_stats.total_entries >= len(test_sessions), f"Cache should contain at least {len(test_sessions)} entries"
        ...
        for session in test_sessions:
            ...
            assert cached_session is not None, f"Session {session.session_id} should be retrievable from cache"
            assert cached_session.client_id == session.client_id, "Client ID should match"
            assert cached_session.current_item == session.current_item, "Task description should match"
        ...
        print(f"  📊 Expecting to restore {expected_restore_count} sessions for 'default' client")
        ...
        assert len(session_repository.get_all_sessions()) == 0, "All sessions should be cleared from memory"
        ...
        restored_count = session_sync_service.restore_sessions_from_cache(client_id="default")
        
        print(f"  📈 Restoration completed: {restored_count} sessions restored")
        assert restored_count == expected_restore_count, f"Expected {expected_restore_count} sessions restored, got {restored_count}"
        ...
        assert len(restored_sessions) == expected_restore_count, f"Expected {expected_restore_count} sessions in memory"
        ...
        assert len(default_restored_sessions) == expected_restore_count, f"Expected {expected_restore_count} sessions for default client"
        ...
        assert restored_session_ids == expected_session_ids, "Restored session IDs should match original session IDs"
        
        for restored_session in default_restored_sessions:
            # Find original session
            original_session = next(s for s in default_client_sessions if s.session_id == restored_session.session_id)
            
            # Verify key properties are restored correctly
            assert restored_session.client_id == original_session.client_id, "Client ID should match"
            assert restored_session.current_item == original_session.current_item, "Task description should match"
            assert restored_session.workflow_name == original_session.workflow_name, "Workflow name should match"
            assert restored_session.current_node == original_session.current_node, "Current node should match"
            
            print(f"  ✅ Verified restored session {restored_session.session_id[:8]}... matches original")

        # STEP 7: Test that non-default client sessions were NOT restored
        print("\n🔒 STEP 7: Verifying client isolation...")
        
        other_client_sessions = session_repository.get_sessions_by_client("other_client")
        assert len(other_client_sessions) == 0, "Sessions for 'other_client' should not be restored when restoring 'default' client"
        print("  ✅ Other client sessions correctly isolated")

        print("\n🎉 Integration test completed successfully!")
        print(f"  📊 Final stats: {len(restored_sessions)} sessions restored, {cache_stats.total_entries} cache entries")
    ...
    def test_restoration_with_corrupted_cache_entries(
        self, session_sync_service, cache_manager, session_repository, sample_workflow_def
    ):
        ...
        session_sync_service.sync_session_to_cache(valid_session.session_id, valid_session)
        ...
        try:
            cache_manager._collection.upsert(
                ...
                metadatas=[{"session_id": "corrupted-session-id", "client_id": "default"}]
            )
        except Exception:
        ...
        assert restored_count >= 1, "Should restore at least the valid session despite corruption"
        
        restored_sessions = session_repository.get_sessions_by_client("default")
        valid_restored = any(s.session_id == valid_session.session_id for s in restored_sessions)
    ...
    def test_auto_restore_on_startup_integration(
        self, session_repository, session_sync_service, cache_manager, sample_workflow_def, server_config
    ):
        ...
        # Sync to cache
        session_sync_service.sync_session_to_cache(test_session.session_id, test_session)
        ...
        with patch.object(session_sync_service, '_get_effective_server_config') as mock_config:
            ...
            assert restored_count == 1, f"Expected 1 session restored by auto_restore, got {restored_count}"
            
            restored_sessions = session_repository.get_all_sessions()
            assert len(restored_sessions) == 1, "Should have 1 session restored in memory"
            
            restored_session = list(restored_sessions.values())[0]
            assert restored_session.session_id == test_session.session_id, "Restored session should match original"

...
class TestMCPClientSemanticWorkflow:
    ...
    async def test_end_to_end_workflow_with_cache_persistence(
        ...
        mock_cache_manager: WorkflowCacheManager,
        ...
        for required_tool in required_tools:
            assert required_tool in tool_names, (
                f"Missing required tool: {required_tool}"
            )
    ...
    async def test_cache_restoration_after_server_restart(
        ...
        retrieved_state = mock_cache_manager.retrieve_workflow_state(
            "restoration-test-session"
        )
...
class TestMCPClientIntegrationErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_cache_unavailable_graceful_degradation(self, mcp_server_with_cache):
    ...
    async def test_semantic_search_with_empty_cache(
        ...
        results = mock_cache_manager.semantic_search(
            search_text="non-existent workflow data", max_results=5, min_similarity=0.1
        )

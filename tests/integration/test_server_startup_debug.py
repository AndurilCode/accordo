...
def test_server_startup_auto_restore_flow():
    ...
    try:
        ...
        print("✅ Successfully imported auto_restore_sessions_on_startup from session_manager")
        
        # Mock the config service to enable cache mode
        with patch('dev_workflow_mcp.utils.session_manager._get_effective_server_config') as mock_config:
            mock_config.return_value = MagicMock(enable_cache_mode=True)
            
            with patch('dev_workflow_mcp.utils.session_manager._ensure_services_initialized'):
                with patch('dev_workflow_mcp.services.get_session_sync_service') as mock_get_service:
    ...
    try:
        from dev_workflow_mcp.services.session_sync_service import SessionSyncService
        ...
        with patch.object(sync_service, '_get_effective_server_config') as mock_config:
            ...
            with patch.object(sync_service, 'restore_sessions_from_cache') as mock_restore:
                mock_restore.return_value = 5
                
                print("🚨 Calling session sync service auto_restore_sessions_on_startup() directly...")
                result = sync_service.auto_restore_sessions_on_startup()
                print(f"✅ Result: {result}")
                
                # Verify delegation worked
                mock_restore.assert_called_once()
                print("✅ Confirmed session sync service auto_restore delegates to restore_sessions_from_cache")
    ...
    try:
        ...
        if "get_session_sync_service" in source:
            print("✅ Confirmed this is the delegation function that calls session sync service")
        else:
    ...
    try:
        from dev_workflow_mcp.utils.session_manager import _ensure_services_initialized
        from dev_workflow_mcp.services import get_session_sync_service
        ...
        if hasattr(sync_service, 'auto_restore_sessions_on_startup'):
            ...
            print("❌ WARNING: Session sync service missing auto_restore_sessions_on_startup method!")
    ...
    try:
        ...
        print("🚨 About to call auto_restore_sessions_on_startup() with real implementation...")
    ...
    try:
        ...
        from dev_workflow_mcp.services.config_service import (
            ServerConfiguration, WorkflowConfiguration, PlatformConfiguration, 
            EnvironmentConfiguration, initialize_configuration_service, ConfigurationService
        )
        from dev_workflow_mcp.services.dependency_injection import register_singleton
        from dev_workflow_mcp.services import initialize_cache_service
        from dev_workflow_mcp.utils.session_manager import auto_restore_sessions_on_startup
        ...
        server_config = ServerConfiguration(
            ...
            cache_db_path=str(Path.cwd() / ".workflow-commander" / "cache"),
        ...
        if config:
            print(f"🚨 Config enable_cache_mode: {getattr(config, 'enable_cache_mode', None)}")

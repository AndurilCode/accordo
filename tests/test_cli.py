...
class TestConfigureCommand:
    ...
    def test_configure_non_interactive_success(self, cli_runner, temp_config_dir):
        ...
        with patch(
            "workflow_commander_cli.handlers.cursor.CursorHandler.add_server",
            return_value=True,
        ):
            result = cli_runner.invoke(
                app,
    ...
    def test_configure_interactive_keyboard_interrupt(self, cli_runner):
        ...
        with patch(
            "workflow_commander_cli.utils.prompts.select_platform",
...
class TestPrompts:
    ...
    def test_select_platform_valid_choice(self):
        ...
        with (
            patch(
                "workflow_commander_cli.utils.prompts.typer.prompt", return_value=1
            ) as mock_prompt,
    ...
    def test_select_platform_invalid_then_valid(self):
        ...
        with (
            patch(
                "workflow_commander_cli.utils.prompts.typer.prompt", side_effect=[5, 2]
            ) as mock_prompt,
            patch("workflow_commander_cli.utils.prompts.typer.secho"),
            patch("workflow_commander_cli.utils.prompts.typer.echo"),
        ):
    ...
    def test_get_workflow_commander_details_default(self):
        ...
        with (
            patch(
                "workflow_commander_cli.utils.prompts.typer.prompt",
                side_effect=["workflow-commander", 1],
            ),  # Server name, basic template
            patch(
                "workflow_commander_cli.utils.prompts.typer.confirm", return_value=False
            ),  # Don't customize
            patch("workflow_commander_cli.utils.prompts.typer.secho"),
            patch("workflow_commander_cli.utils.prompts.typer.echo"),
        ):
    ...
    def test_get_workflow_commander_details_custom(self):
        ...
        with (
            ...
            patch(
                "workflow_commander_cli.utils.prompts.typer.confirm",
                side_effect=[
